...
import type { Position, SignalState, UserProfile, WatchItem } from '../types/market';

type EditorTarget = { kind: 'position'; value?: Position } | { kind: 'watch'; value?: WatchItem };

export function EditorModal({ target, users, defaultUserId, onClose, onSave }: {
  target: EditorTarget;
  users: UserProfile[];
  ...
  useEffect(() => {
    ...
    if (isPosition) {
      ...
      setForm({ id: p?.id ?? crypto.randomUUID(), symbol: p?.symbol ?? '', name: p?.name ?? '', userId: p?.userId ?? defaultUserId, shares: String(p?.shares ?? ''), cost: String(p?.cost ?? ''), price: String(p?.price ?? '') });
    } else {
      const w = value as WatchItem | undefined;
      setForm({ symbol: w?.symbol ?? '', name: w?.name ?? '', userId: w?.userId ?? defaultUserId, price: String(w?.price ?? ''), changePct: String(w?.changePct ?? ''), group: w?.group ?? '', score: String(w?.score ?? 5), state: w?.state ?? '观察', support: w?.support ?? '', trigger: w?.trigger ?? '', invalidation: w?.invalidation ?? '', note: w?.note ?? '' });
    }
  }, [defaultUserId, isPosition, target.value]);

  const set = (key: string, value: string) => setForm(current => ({ ...current, [key]: value }));
  const submit = (event: FormEvent) => {
    event.preventDefault();
    const owner = users.find(user => user.id === form.userId)?.name ?? 'JJ';
    if (isPosition) onSave({ id: form.id, symbol: form.symbol.trim(), name: form.name.trim(), owner, userId: form.userId, shares: Number(form.shares), cost: Number(form.cost), price: Number(form.price) });
    else onSave({ symbol: form.symbol.trim(), name: form.name.trim(), userId: form.userId, price: Number(form.price), changePct: Number(form.changePct), group: form.group.trim(), score: Number(form.score), state: form.state as SignalState, support: form.support.trim(), trigger: form.trigger.trim(), invalidation: form.invalidation.trim(), note: form.note.trim() });
  };
  ...
  return <div className="modal-backdrop" onMouseDown={e => e.target === e.currentTarget && onClose()}>
    <form className="modal" onSubmit={submit}>
      ...
      <div className="form-grid">
        <label className="full-field"><span>归属用户</span><select value={form.userId ?? defaultUserId} onChange={e => set('userId', e.target.value)}>{users.filter(user => !user.archived).map(user => <option key={user.id} value={user.id}>{user.name}</option>)}</select></label>
