...
export function StrategyImporter({ users, selectedUserId, workflows, alerts, watchlist, onClose, onImport }: {
  ...
  onClose: () => void; onImport: (pack: StrategyPackage, selections: StrategySelection[], userId: string, metadata: { repairs: string[] }) => void;
}) {
  const [userId, setUserId] = useState(selectedUserId === 'all' ? '' : selectedUserId);
  const [raw, setRaw] = useState('');
  const [repairAudit, setRepairAudit] = useState<{ key: string; repairs: string[] }>({ key: '', repairs: [] });
  ...
  const packageKey = parsed.pack ? `${parsed.pack.date}\u0000${parsed.pack.title}` : '';
  const auditRepairs = repairAudit.key === packageKey ? repairAudit.repairs : parsed.repairs;
  const changeRaw = (value: string) => {
    setRaw(value); setSelections({}); setConfirmed(false); setError('');
    if (!value.trim()) { setRepairAudit({ key: '', repairs: [] }); return; }
    try {
      const next = parseStrategyInput(value);
      const key = `${next.pack.date}\u0000${next.pack.title}`;
      setRepairAudit(current => next.repairs.length ? { key, repairs: next.repairs } : current.key === key ? current : { key, repairs: [] });
    } catch {
      // Keep the last valid audit while the user is midway through an edit.
    }
  };
  ...
  return <div className="modal-backdrop chat-import-backdrop"><section className="modal chat-import-modal strategy-import" role="dialog" aria-modal="true" aria-label="AI 策略快导">
    <div className="modal-head"><div><span className="eyebrow">STRATEGY INTAKE · V1.6</span><h3>AI 策略快导</h3><p>一次粘贴，自动检查格式并建立不可变来源档案，再分别确认任务与价格边界。不会读取 AI 账户或自动下单。</p></div><button className="ghost-btn" onClick={onClose}>关闭</button></div>
    ...
    <div className="chat-import-grid"><div className="chat-source-panel chat-paste-fields">
      ...
      <label>策略 JSON<textarea aria-label="策略 JSON" value={raw} onChange={event => changeRaw(event.target.value)} placeholder="把 AI 按提示词生成的 JSON 粘贴到这里，也支持 JSON 代码块。"/></label>
    ...
    <div className="strategy-confirm">
      ...
      <p>任务按当天阶段、标题和完整详情去重；提醒按账户、代码、方向、价格去重（包括暂停规则）。本次确认后会保存策略快照和产物来源；不会覆盖旧记录。</p>
    ...
    <div className="modal-actions"><button className="ghost-btn" onClick={onClose}>取消</button><button className="primary-btn" disabled={!owner || !confirmed || !preview || !(preview.addedTasks + preview.addedAlerts)} onClick={() => {
      ...
      try { onImport(parsed.pack, choices, userId, { repairs: auditRepairs }); } catch (error) { setError((error as Error).message); setConfirmed(false); }
    }}>确认导入策略</button></div>
  </section></div>;
}
