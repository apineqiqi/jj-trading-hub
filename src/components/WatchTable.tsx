  ...
  return <div className="card table-card">
    ...
    {items.map(x => <div className="watch-row" key={x.symbol}>
      <div className="watch-grid main-row">
        <span><b>{x.name}</b><small>{x.symbol} · {x.group}{showOwners && <em className="owner-tag" style={{ '--owner-color': userMap.get(x.userId ?? '')?.color } as CSSProperties}>{userMap.get(x.userId ?? '')?.name ?? 'JJ'}</em>}</small></span>
