      ...
      <div className="alert-list">
        {filtered.length ? filtered.map(rule => {
          ...
          return <article className={`${hit ? 'triggered' : ''} ${!rule.enabled ? 'paused' : ''}`} key={rule.id}>
            ...
            <div className="alert-copy"><div><b>{hidden ? '观察标的' : rule.name}</b><em>{userMap.get(rule.userId)?.name ?? 'JJ'}</em><span>{rule.direction === 'above' ? '上穿' : '下破'} {hidden ? '••••' : rule.target.toFixed(2)}</span></div><p>{rule.label}</p><small>现价 {hidden ? '••••' : item?.price.toFixed(2) ?? '—'} · {rule.enabled ? hit ? '条件已命中' : '等待价格进入边界' : '监控已暂停'}</small></div>
